...
const EditProfileModal = ({ isOpen, onClose, onSaved }: EditProfileModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setIsLoading(true);
    try {
      const res = await fetchWithAuth('/user/me', { method: 'PUT', body: JSON.stringify(payload) });
      if (!res.ok) {
        const data = await res.json();
        setError(data.msg || 'Something went wrong.');
        return;
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-[520px] bg-white rounded-3xl p-8 shadow-[0_32px_64px_rgba(0,0,0,0.12)] border border-gray-100 z-10">
        <button
          onClick={onClose}
          className="absolute top-5 right-5 w-8 h-8 flex items-center justify-center rounded-xl text-zinc-400 hover:text-zinc-700 hover:bg-gray-100 transition-colors"
        >
        ...
        <h2 className="text-xl font-extrabold text-[#1a1a1a] mb-1 tracking-tight">Edit Profile</h2>
        <p className="text-[13px] text-zinc-500 mb-6">Update your public profile information.</p>

        <form className="space-y-4" onSubmit={handleSubmit}>
          <div className="space-y-1.5">
            <label className="text-[13px] font-semibold text-[#1a1a1a] pl-1">Name *</label>
            <Input
              name="name"
          ...
          <div className="space-y-1.5">
            <label className="text-[13px] font-semibold text-[#1a1a1a] pl-1">Bio</label>
            <textarea
              name="bio"
          ...
          <div className="pt-1">
            <p className="text-[11px] font-bold uppercase tracking-widest text-zinc-400 mb-3">Socials</p>
          ...
          {error && <p className="text-[13px] text-red-500 font-medium pl-1">{error}</p>}
  ...
  const fetchProjects = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Filter to only this user's projects (owner may be populated object or bare string)
        ...
        <div className="bg-white/70 backdrop-blur-xl rounded-3xl border border-white shadow-[0_20px_40px_rgba(0,0,0,0.04)] p-8 sm:p-10 flex flex-col sm:flex-row items-start sm:items-center gap-6">
          ...
          <div className="shrink-0 flex gap-2">
            <button
              ...
              className="h-9 px-4 rounded-xl border border-gray-200 text-[13px] font-semibold text-zinc-500 hover:text-[#E67A62] hover:border-[#E67A62]/30 hover:bg-[#E67A62]/5 transition-colors"
            >
              Edit Profile
            </button>
            <button
              onClick={logout}
              className="h-9 px-4 rounded-xl border border-gray-200 text-[13px] font-semibold text-zinc-500 hover:text-red-500 hover:border-red-200 hover:bg-red-50 transition-colors"
            >
              Sign Out
            </button>
          </div>
            ...
            {projects.map((project) => (
              ...
              >
                ...
                <div className="flex items-center gap-3 mb-5 text-[12px] font-semibold">
                  <button
                    onClick={(e) => handleUpvote(project._id, e)}
                    className={`flex items-center gap-1.5 transition-colors ${
                      project.upvotes?.includes(user?._id || '')
                        ? 'text-[#E67A62]'
                        : 'text-zinc-500 hover:text-zinc-700'
                    }`}
                  >
                    <svg viewBox="0 0 24 24" fill={project.upvotes?.includes(user?._id || '') ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-3.5 h-3.5">
